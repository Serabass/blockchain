...
describe("Hello function", () => {
    ...
    it('parse', async () => {
        let s = `48ad43e5d230f64e7fbd9285c3da61579b0766982c124bec9907d8356b3c0e6c|DSU005|1558507200000
7dceb632f978adc3483d4ecba7f064c19f68811156515d2f90ee162741f49eaf|DSU001|1558496400000`;

        let b = await StringBlockChain.fromString(s);
        expect(b.transactions[0].data).toBe('500USD');
        expect(b.transactions[1].data).toBe('100USD');
    });

    it('parse wrong', async () => {
        let s = `48ad43e5d230f64e7fbd9285c3da61579b0766982c124bec9907d8356b3c0e6c|DSU008|1558507200000
7dceb632f978adc3483d4ecba7f064c19f68811156515d2f90ee162741f49eaf|DSU001|1558496400000`;

        expect(StringBlockChain.fromString(s)).rejects.toMatch(/Transaction is incorrect/);
    });
});
